...
Let's start with a working example which is prepared in the public GitHub repository https://github.com/xstefank/pa165-quarkus/tree/main/seminar-06/muni-chat-service-quarkus (thanks to Martin Kuba for providing this application which was transformed to Quarkus).
...
[ApiResourceTest.java](server/src/test/java/cz/muni/pa165/generated/server/ApiResourceTest.java), it can be
...
./mvnw quarkus:dev
```
then visit the Swagger UI for the service at http://localhost:8080/q/swagger-ui and call the hello
   ...
   schemas:
      User:
         ...
         properties:
            ...
            age: { type: [ "integer", "null" ], description: age in years, example: 20 }
...
that may be null. In OpenAPI 3.0, use `type: string, nullable: true`.
  ...
  /api/users:
    post:
      ...
      parameters:
        ...
        - { name: height, in: query, required: false, schema: { type: integer, format: int32 }, description: "user's height in cm" }
        - { name: age, in: query, required: false, schema: { type: integer, format: int32 }, description: "user's age in years" }
      responses:
        "200":
          description: OK
          content:
...
Regenerate the classes by issuing `mvn clean install`, then implement the operation in [MyApiResource.java](server/src/main/java/cz/muni/pa165/generated/server/MyApiResource.java).
...
public class MyApiResource implements ApiResource {
    //...
    @Override
    public User createUser(String name, Integer height, Double age) {
       User user = new User();
       user.setName(name);
       user.setHeight(height);
       user.setAge(age);
       user.setVampire(false);
       user.setHairColor(User.HairColor.DARK);
       user.setChildrenNames(List.of("Petr", "Zuzana"));
       Log.debugf("createUser(%s, %s, %s) returns %s", name, height, age, user);
       return user;
    }
}
```

Run the server again with `./mvnw quarkus:dev` and call the operation from the Swagger UI at http://localhost:8080/q/swagger-ui/index.html
...
```
Run the client by issuing `./mvnw quarkus:dev` in a new terminal window in the client folder (the server must be still running):
```bash
cd client
mvn
```
...
In the class [ApiResourceTest.java](server/src/test/java/cz/muni/pa165/generated/server/ApiResourceTest.java), write a test
for your operation.
